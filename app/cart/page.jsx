...
const Cart = () => {
  ...
  return (
    <>
    <div className="border rounded-lg p-4 shadow-md w-[53pc] relative left-[15pc]">
      <h2 className="text-lg font-semibold mb-4 text-center">Your Cart</h2>
      {cart.length === 0 ? (
        <p>Your cart is empty {cart.length}</p>
      ) : (
        <ul>
          {cart.map((product) => (
            <li
            ...
            >
              <div>
                ...
                <Image width={85} height={130} className="ml-7 object-cover justify-center" src={product.pic} alt="img" />
    ...
    <Link className="left-[15pc] top-6 relative" href='/' >Go to Home Page </Link>
    </>
  );
};
